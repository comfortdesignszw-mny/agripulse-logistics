...
export default function SADCFeedHub({ currentUser }: SADCFeedHubProps) {
  const [hubTab, setHubTab] = useState<
    "all" | "requests" | "carriers" | "bids" | "classifieds"
  >("all");
  const [searchQuery, setSearchQuery] = useState("");
  const [sortBy, setSortBy] = useState<"newest" | "price_low" | "nearest">(
    "newest",
  );
  ...
  const transportRequests =
    useLiveQuery(() => db.transportRequests.reverse().sortBy("createdAt")) ||
    [];
  const bids = useLiveQuery(() => db.bids.reverse().sortBy("timestamp")) || [];
  const users = useLiveQuery(() => db.users.toArray()) || [];
  const adverts =
    useLiveQuery(() => db.adverts.reverse().sortBy("timestamp")) || [];
  ...
  const getTimelineItems = () => {
    ...
    bids.forEach((bid) => {
      const associatedReq = transportRequests.find(
  ...
  const getFilteredRequests = () => {
    return transportRequests.filter(
      (req) =>
  ...
  const getFilteredClassifieds = () => {
    return adverts.filter((a) => {
      ...
      const authorLoc = authorUser
        ? authorUser.location || authorUser.farmAddress || ""
        : "";
      return (
  ...
  const isNearest = (targetText?: string) => {
    ...
    const userLoc = (currentUser.location || currentUser.farmAddress || "")
      .toLowerCase()
      .trim();
    if (!userLoc) return false;
    const keywords = userLoc.split(/[\s,]+/);
    return keywords.some(
      (kw) => kw.length > 2 && targetText.toLowerCase().includes(kw),
    );
  };
    ...
    <div className="space-y-6 text-left font-sans">
      ...
      <div className="bg-gradient-to-r from-emerald-700 to-indigo-800 text-white rounded-3xl p-6 shadow-sm border border-emerald-600/30 relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-2">
          ...
          <h1 className="text-2xl font-black tracking-tight font-sans">
            SADC Logistics & Discovery Feed
          </h1>
          <p className="text-xs text-emerald-100 max-w-md leading-relaxed">
            Interconnecting local smallholders, regional truckers, and wholesale
            market agents instantly. Search harvests, negotiate contracts, and
            inspect driver verification lists.
          </p>
        </div>
      </div>
      ...
      <div className="flex border-b border-slate-200 bg-slate-50/50 p-1.5 rounded-2xl gap-1 overflow-x-auto scrollbar-none z-20">
        ...
        >
          <RefreshCw
            size={13}
            className={hubTab === "all" ? "text-emerald-600" : ""}
          />
          Combined Feed
        </button>
        ...
        >
          <FileText
            size={13}
            className={hubTab === "requests" ? "text-emerald-600" : ""}
          />
        ...
        >
          <Truck
            size={13}
            className={hubTab === "carriers" ? "text-indigo-600" : ""}
          />
        ...
        >
          <Gavel
            size={13}
            className={hubTab === "bids" ? "text-amber-600" : ""}
          />
        ...
        >
          <Megaphone
            size={13}
            className={hubTab === "classifieds" ? "text-slate-600" : ""}
          />
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-white p-4 rounded-2xl border border-slate-200 shadow-3xs">
        <div className="text-left">
          ...
          <h3 className="font-extrabold text-slate-800 text-xs sm:text-sm">
            {hubTab === "all" &&
              `Showing ${getSortedTimeline().length} Activity Events`}
            {hubTab === "requests" &&
              `Showing ${getSortedRequests().length} Open Cargo Sacks`}
            {hubTab === "carriers" &&
              `Showing ${getSortedCarriers().length} Registered Driver Hubs`}
            {hubTab === "bids" &&
              `Showing ${getSortedBids().length} Transit Negotiation Bids`}
            {hubTab === "classifieds" &&
              `Showing ${getSortedClassifieds().length} Verified SADC Ads`}
          </h3>
        </div>
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          <select
            value={sortBy}
            onChange={(e) =>
              setSortBy(e.target.value as "newest" | "price_low" | "nearest")
            }
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center px-1">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                Chronological Feed Pipeline ({getSortedTimeline().length}{" "}
                events)
              </span>
            </div>
            ...
            {getSortedTimeline().length === 0 ? (
              <div className="bg-white p-12 rounded-3xl border border-slate-200 text-center text-xs text-slate-400 flex flex-col items-center justify-center space-y-4">
                ...
                <span>
                  No active discovery signals matched your search criteria. Try
                  modifying your keyphrase or register a listing.
                </span>
              </div>
            ) : (
              <div className="space-y-3">
                {getSortedTimeline().map((item) => (
                  <div
                    key={item.id}
                    className="bg-white rounded-2xl border border-slate-200 p-4 shadow-3xs hover:border-slate-300 transition-all flex gap-3 text-left"
                  >
                    ...
                    <div className="flex-1 min-w-0 space-y-1">
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        ...
                        <span
                          className={`text-[8px] font-bold px-2 py-0.5 rounded border ${item.badgeColor}`}
                        >
                          {item.metaBadge}
                        </span>
                      </div>
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            {(() => {
                              const authorUser = users.find(
                                (u) => u.name === item.author,
                              );
                              if (authorUser) {
                                return (
                                  <StarRating
                                    rating={authorUser.ratingValue ?? 5.0}
                                    count={authorUser.ratingCount ?? 2}
                                    size={10}
                                  />
                                );
                              }
                          ...
                          <div className="pt-2">
                            {successBids[item.details.id] ? (
                              <div className="bg-emerald-50 border border-emerald-200 text-emerald-850 text-[10px] font-bold p-2 rounded-xl flex items-center gap-2">
                                <Check size={14} /> Quick bid registered
                                successfully!
                              </div>
                            ) : (
                              <div className="bg-slate-50 border border-slate-150 p-2 rounded-xl flex flex-col sm:flex-row items-center gap-2 justify-between">
                                <div className="flex items-center gap-1 w-full sm:w-auto">
                                  ...
                                  <input
                                    ...
                                    onChange={(e) =>
                                      setQuickBidPrices((prev) => ({
                                        ...prev,
                                        [item.details.id]: e.target.value,
                                      }))
                                    }
          ...
          <div className="space-y-4">
            <div className="bg-slate-50 border border-slate-200 p-4 rounded-2xl flex items-start gap-2.5">
              ...
              <p className="text-[11px] text-slate-600 leading-relaxed">
                Below are the active smallholder harvests needing trucking
                services in SADC. SADC-registered carriers can coordinate
                directly or offer customized counter-pricing estimates using the
                bid utility.
              </p>
            </div>
            ...
            {getSortedRequests().length === 0 ? (
              <div className="bg-white p-12 rounded-3xl border border-slate-200 text-center text-xs text-slate-400 flex flex-col items-center justify-center space-y-4">
                <ShieldAlert size={28} className="text-slate-300" />
                <span>
                  Savannah harvest logistics is fully deployed. No open cargo
                  requests right now.
                </span>
                <button
                  onClick={() =>
                    alert(
                      "Please proceed to your dashboard or Farmer View to register a new harvest cargo allocation request.",
                    )
                  }
              ...
              <div className="space-y-3">
                {getSortedRequests().map((req) => {
                  const associatedBids = bids.filter(
                    (b) => b.requestId === req.id,
                  );
                  const alreadyBidded = associatedBids.some(
                    (b) => b.bidderId === currentUser.id,
                  );

                  return (
                    <div
                      key={req.id}
                      className="bg-white rounded-2xl p-4 border border-slate-205 shadow-3xs relative overflow-hidden text-left flex flex-col md:flex-row gap-4"
                    >
                      {alreadyBidded && (
                        ...
                        <div className="flex justify-between items-start gap-4">
                          <div>
                            ...
                            <h3 className="font-extrabold text-slate-900 text-sm mt-1">
                              {req.cropName} yield load
                            </h3>
                            <div className="text-[10px] text-slate-450 mt-0.5 flex items-center gap-1.5 flex-wrap">
                              <span>
                                Offered by smallholder:{" "}
                                <strong className="text-slate-700 font-bold">
                                  {req.farmerName}
                                </strong>
                              </span>
                              {(() => {
                                const shipperUser = users.find(
                                  (u) =>
                                    u.name === req.farmerName ||
                                    u.id === req.farmerId,
                                );
                                if (shipperUser) {
                                  return (
                                    <StarRating
                                      rating={shipperUser.ratingValue ?? 5.0}
                                      count={shipperUser.ratingCount ?? 2}
                                      size={10}
                                    />
                                  );
                                }
                        ...
                        <div className="flex items-center justify-between pt-1 border-t border-slate-100">
                          ...
                          currentUser.userRole !== "Farmer" ? (
                            ...
                            <span className="text-[9px] italic text-slate-400">
                              Managed under farmer workstation
                            </span>
                          )}
                        </div>
                      </div>
        ...
        {hubTab === "carriers" && (
          <div className="space-y-4">
            <div className="bg-slate-50 border border-slate-200 p-4 rounded-2xl flex items-start gap-2.5">
              ...
              />
              <p className="text-[11px] text-slate-600 leading-relaxed">
                These are active shipping operators and fleet carriers within
                the regional SADC Alliance, complete with location bases and
                verification compliance logs. Tap to contact or invite to cargo
                allocations.
              </p>
            </div>
            ...
            {getSortedCarriers().length === 0 ? (
              <div className="bg-white p-12 rounded-3xl border border-slate-200 text-center text-xs text-slate-400 flex flex-col items-center justify-center space-y-4">
                <ShieldAlert size={28} className="text-slate-300" />
                <span>
                  No active transporters or shipping registries found matching
                  your criteria.
                </span>
                <button
                  onClick={() =>
                    alert(
                      "Navigate to 'Directory Registry' to find verified cargo drivers and shipping services.",
                    )
                  }
              ...
              <div className="grid grid-cols-1 gap-3">
                {getSortedCarriers().map((carrier) => (
                  <div
                    key={carrier.id}
                    className="bg-white rounded-2xl border border-slate-200 p-4 shadow-3xs flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 text-left"
                  >
                    <div className="flex gap-3">
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          <span
                            className={`text-[8px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider ${carrier.verificationStatus === "Verified" ? "bg-indigo-600 text-white border-indigo-700" : "bg-slate-100 text-slate-550"}`}
                          >
                            {carrier.verificationStatus === "Verified"
                              ? "✓ VERIFIED DRIVER"
                              : "SADC REGISTRY PENDING"}
                          </span>
                        </div>
                        ...
                        <div className="mt-0.5 mb-1 bg-slate-50/50 inline-block px-2 py-0.5 rounded border border-slate-100">
                          <StarRating
                            rating={carrier.ratingValue ?? 4.8}
                            count={carrier.ratingCount ?? 4}
                            size={10}
                          />
                        </div>
                        ...
                        <p className="text-[10px] text-slate-500 font-semibold flex items-center gap-1">
                          <MapPin size={11} className="text-slate-400" />{" "}
                          Carrier Base:{" "}
                          {carrier.location || "Coordinating hub location"}
                        </p>
                        <p className="text-[9px] text-slate-400 uppercase tracking-widest font-bold">
                          Logistics SADC Alliance Network Partner
                        </p>
                      </div>
                    </div>
        ...
        {hubTab === "bids" && (
          <div className="space-y-4">
            <div className="bg-slate-50 border border-slate-200 p-4 rounded-2xl flex items-start gap-2.5">
              ...
              />
              <p className="text-[11px] text-slate-600 leading-relaxed">
                Visualizing interactive negotiation boards and tender bids
                placed on active SADC crops. Rates reflect local shipping price
                checks.
              </p>
            </div>
            ...
            {getSortedBids().length === 0 ? (
              <div className="bg-white p-12 rounded-3xl border border-slate-200 text-center text-xs text-slate-400 flex flex-col items-center justify-center space-y-4">
                <ShieldAlert size={28} className="text-slate-300" />
                <span>
                  You have not initiated any bids or negotiation boards yet.
                  Browse active cargo orders to place your first bid!
                </span>
                <button
                  onClick={() => setHubTab("requests")}
                  className="bg-emerald-600 hover:bg-emerald-700 text-white text-[10px] font-black uppercase tracking-wider px-4 py-2 rounded-xl transition-colors shrink-0 shadow-3xs cursor-pointer"
              ...
              <div className="space-y-3">
                {getSortedBids().map((bid) => {
                  const correlatedReq = transportRequests.find(
                    (r) => r.id === bid.requestId,
                  );
                  ...
                  return (
                    <div
                      key={bid.id}
                      className="bg-white rounded-2xl p-4 border border-slate-200 shadow-3xs text-left"
                    >
                      <div className="flex justify-between items-start flex-wrap gap-2">
                        <div>
                          <span
                            className={`text-[8px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wider ${bid.bidderRole === "Transporter" ? "bg-indigo-50 text-indigo-700 border-indigo-150" : "bg-amber-50 text-amber-700 border-amber-150"}`}
                          >
                            {bid.bidderRole} Carrier Rate
                          </span>
                          <h4 className="font-extrabold text-slate-800 text-sm mt-1">
                            {bid.bidderName || `Carrier ${bid.bidderId}`}
                          </h4>
                          ...
                          <div className="mt-0.5 mb-1.5 block">
                            {(() => {
                              const bidderUser = users.find(
                                (u) =>
                                  u.name === bid.bidderName ||
                                  u.id === bid.bidderId,
                              );
                              if (bidderUser) {
                                return (
                                  <StarRating
                                    rating={bidderUser.ratingValue ?? 4.8}
                                    count={bidderUser.ratingCount ?? 2}
                                    size={10}
                                  />
                                );
                              }
                          ...
                          <p className="text-[10px] text-indigo-700 font-extrabold mt-0.5">
                            Proposed rate allocation: ${bid.offerPrice} USD
                          </p>
                        </div>
                        <div className="text-right">
                          <span
                            className={`text-[9px] font-black px-2.5 py-1 rounded shadow-3xs uppercase ${bid.status === "Accepted" ? "bg-emerald-100 text-emerald-800 border-emerald-200" : bid.status === "Rejected" ? "bg-rose-100 text-rose-800" : "bg-amber-100 text-amber-800"}`}
                          >
                            {bid.status}
                          </span>
                        </div>
                        ...
                        <div className="mt-3.5 bg-slate-50 border border-slate-150 p-2.5 rounded-xl flex items-center justify-between text-[11px] font-medium text-slate-600 flex-wrap gap-2">
                          <div className="flex items-center gap-1.5">
                            <span className="font-extrabold text-slate-700">
                              Ref Yield: {correlatedReq.cropName}
                            </span>
                            <span>•</span>
                            <span>
                              Volume: {correlatedReq.quantity}{" "}
                              {correlatedReq.unit}
                            </span>
                          </div>
                          <div>
                            <span>
                              Route: {correlatedReq.origin} →{" "}
                              {correlatedReq.destination}
                            </span>
                          </div>
                        </div>
                      ) : (
                        <p className="text-[9px] text-slate-400 mt-2">
                          Correlated agricultural cargo record has resolved or
                          cleared.
                        </p>
                      )}
                    </div>
          ...
          <div className="space-y-4">
            {getSortedClassifieds().length === 0 ? (
              <div className="bg-white p-12 rounded-3xl border border-slate-200 text-center text-xs text-slate-400 flex flex-col items-center justify-center space-y-4">
                <ShieldAlert size={28} className="text-slate-300" />
                <span>
                  Classified Hub is standing by. No community advertisements
                  registered yet today.
                </span>
                <button
                  onClick={() =>
                    alert(
                      "Please proceed to your role-specific dashboard (Farmer, Transporter, or Dealer) to register a new Classified Ad!",
                    )
                  }
                  className="bg-emerald-600 hover:bg-emerald-700 text-white text-[10px] font-black uppercase tracking-wider px-4 py-2 rounded-xl transition-colors shrink-0 shadow-3xs cursor-pointer"
                >
              ...
              <div className="space-y-4">
                {getSortedClassifieds().map((adv) => (
                  <div
                    key={adv.id}
                    className="bg-white rounded-2xl border border-slate-200 p-4 shadow-3xs text-left"
                  >
                    <div className="flex justify-between items-start gap-3">
                      <div>
                        <span
                          className={`text-[8px] font-bold px-2.5 py-0.5 rounded uppercase tracking-wider ${adv.type === "ProduceSale" ? "bg-emerald-100 text-emerald-700 border-emerald-200" : adv.type === "TransportOffer" ? "bg-indigo-100 text-indigo-700" : "bg-amber-100 text-amber-700"}`}
                        >
                          {adv.type === "ProduceSale"
                            ? "Farmer Harvest Sale"
                            : adv.type === "TransportOffer"
                    ...
                    <div className="grid grid-cols-3 gap-2 mt-3.5">
                      {(adv.images || []).map((imgUrl: string, idx: number) => (
                        <div
                          key={idx}
                          className="rounded-xl overflow-hidden border border-slate-200 h-32"
                        >
                          <img
                            src={imgUrl}
                            alt={`${adv.title}-${idx}`}
                    ...
                    <div className="mt-3 pt-3 border-t border-slate-100 flex items-center justify-between text-[10px] text-slate-450 font-semibold flex-wrap gap-2">
                      <div className="flex items-center gap-1.5">
                        <span>
                          Publisher: {adv.authorName} ({adv.authorRole})
                        </span>
                        {(() => {
                          const sellerUser = users.find(
                            (u) =>
                              u.name === adv.authorName ||
                              u.id === adv.authorId,
                          );
                          if (sellerUser) {
                            return (
                              <StarRating
                                rating={sellerUser.ratingValue ?? 5.0}
                                count={sellerUser.ratingCount ?? 2}
                                size={10}
                              />
                            );
                          }
                      ...
                      <div className="flex items-center gap-2">
                        <span>
                          Date: {new Date(adv.timestamp).toLocaleDateString()}
                        </span>
                          ...
                          <button
                            onClick={() =>
                              setSelectedReq({
                                id: adv.id,
                                quantity: "",
                                unit: adv.unitType || "Tonne",
                                cropName: adv.cropName || adv.title,
                                farmerName: adv.authorName,
                              })
                            }
                            className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-3 py-1 rounded shadow-sm text-xs ml-2"
                          >
                            Bid
                          </button>
                        )}
          ...
          <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl relative text-left border border-slate-100">
            ...
            <div className="flex items-center gap-2 mb-1.5">
              ...
              <h3 className="text-base font-extrabold text-slate-850">
                Submit SADC Cargo Bid
              </h3>
            </div>
            <p className="text-xs text-slate-500 leading-normal mb-4">
              You are negotiating transit pricing for{" "}
              <strong className="text-slate-700">
                {selectedReq.quantity} {selectedReq.unit}
              </strong>{" "}
              of{" "}
              <strong className="text-emerald-700">
                {selectedReq.cropName}
              </strong>{" "}
              requested by {selectedReq.farmerName}.
            </p>
            ...
            {bidSuccess ? (
              <div className="bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-bold p-3 rounded-2xl flex items-center gap-2.5 animate-pulse">
                <Check size={16} /> Fast Carrier Bid Registered Locally!
                Syncing...
              </div>
            ) : (
