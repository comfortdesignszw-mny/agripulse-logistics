...
import { 
  Search, Shield, Phone, Mail, MapPin, BadgeCheck, 
  UserCheck, AlertTriangle, RefreshCw, Layers, Sparkles 
} from 'lucide-react';
...
export default function SADCMembers({ currentUser }: SADCMembersProps) {
  ...
  const [roleFilter, setRoleFilter] = useState<'all' | 'Farmer' | 'Transporter' | 'Dealer'>('all');
  ...
  const handleRateMember = async (stars: number) => {
    ...
    setSelectedMember(prev => prev ? { ...prev, ratingValue: roundedVal, ratingCount: newCount } : null);
    setRatingFeedback(`Rated ${stars} Star${stars > 1 ? 's' : ''}! Trust score updated.`);
  ...
  const filteredMembers = members.filter(m => {
    const matchesSearch = 
      ...
      (m.farmAddress && m.farmAddress.toLowerCase().includes(searchQuery.toLowerCase())) ||
    ...
    <div className="space-y-6 text-left font-sans">
      ...
      <div className="bg-gradient-to-r from-indigo-750 to-slate-900 text-white rounded-3xl p-6 shadow-sm border border-slate-700/30 relative overflow-hidden">
        <div className="relative z-10 space-y-2">
          ...
          <h1 className="text-2xl font-black tracking-tight">SADC Verified Directory</h1>
          <p className="text-xs text-indigo-100 max-w-md">
            Look up regional trade partners, examine driver KYC status, and request directly. Security protocol enforces read-only access on external accounts.
          </p>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-3xs space-y-3">
            ...
            <div className="flex gap-1 overflow-x-auto pb-1">
              {(['all', 'Farmer', 'Transporter', 'Dealer'] as const).map(role => (
                <button 
                  key={role}
          ...
          <div className="space-y-3">
                  ...
                  <div className="flex gap-3">
                    <div className="w-10 h-10 rounded-full bg-slate-50 border border-slate-200 flex items-center justify-center text-lg overflow-hidden flex-shrink-0">
                      {member.profileImage ? (
                        ...
                        member.userRole === 'Farmer' ? '🌾' : member.userRole === 'Transporter' ? '🚚' : '🏬'
                      )}
                    </div>
                    <div>
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <h4 className="font-extrabold text-slate-800 text-xs sm:text-sm">{member.name}</h4>
                        {isSelf && (
                          <span className="bg-slate-100 text-slate-700 text-[8px] px-1.5 font-bold uppercase rounded">My Acc</span>
                        )}
                      </div>
                      <p className="text-[10px] text-slate-500 font-semibold">{member.userRole} • {member.location || member.farmAddress || 'Regional Base'}</p>
                      
                      {/* Trust rating visualization */}
                      <div className="mt-1">
                        <StarRating rating={member.ratingValue ?? 5.0} count={member.ratingCount ?? 2} size={11} />
                      </div>
                    </div>
                  </div>

                  <div className="flex flex-col items-end gap-1 flex-shrink-0">
                    <span className={`text-[8px] font-black px-2 py-0.5 rounded border uppercase ${member.verificationStatus === 'Verified' ? 'bg-indigo-50 text-indigo-700 border-indigo-100' : 'bg-slate-10$ text-slate-500'}`}>
            ...
            {filteredMembers.length === 0 && (
              <div className="bg-white p-12 rounded-2xl border border-slate-200 text-center text-xs text-slate-400">
                No verified agricultural SADC members matched your search criteria.
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-4">
          {selectedMember ? (
            <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-3xs text-left relative overflow-hidden flex flex-col gap-4">
              ...
              <div className="flex flex-col items-center text-center p-4 bg-slate-50 rounded-2xl border border-slate-150">
                <div className="w-20 h-20 rounded-full bg-white border-2 border-indigo-600 shadow-sm overflow-hidden flex items-center justify-center text-3xl mb-3">
                  {selectedMember.profileImage ? (
                    ...
                    selectedMember.userRole === 'Farmer' ? '🌾' : selectedMember.userRole === 'Transporter' ? '🚚' : '🏬'
                  )}
                </div>

                <div className="space-y-1 w-full flex flex-col items-center">
                  <div className="flex items-center justify-center gap-1">
                    <h3 className="font-black text-slate-850 text-base leading-none">{selectedMember.name}</h3>
                  ...
                  <div className="mt-2 flex flex-col items-center bg-white border border-slate-200 rounded-xl px-3 py-1.5 shadow-3xs w-auto">
                    <span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest block mb-0.5">SADC Trust Score</span>
                    <StarRating rating={selectedMember.ratingValue ?? 5.0} count={selectedMember.ratingCount ?? 2} size={15} />
                  </div>
                </div>
              </div>
              ...
              {selectedMember.id !== currentUser.id && (
                <div className="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-100 p-4 rounded-2xl">
                  <h5 className="text-[10px] font-black text-amber-800 uppercase tracking-widest mb-1">Rate this Partner's Trust Service</h5>
                  <p className="text-[10px] text-slate-550 leading-normal mb-2.5">
                    Have you collaborated with {selectedMember.name}? Submit a 5-star validation score to secure network accountability.
                  </p>
              ...
              <div className="space-y-2.5">
                <h4 className="text-[10px] uppercase font-bold text-slate-400 border-b border-slate-100 pb-1">Business Credentials</h4>
                
                {selectedMember.userRole === 'Farmer' && (
                  <>
                    <div className="text-xs">
                      <span className="text-slate-400 font-medium block">Crops Produced:</span>
                      <strong className="text-slate-800 font-bold">{selectedMember.cropSpecializations || 'Maize, Groundnuts'}</strong>
                    </div>
                    <div className="text-xs">
                      <span className="text-slate-400 font-medium block">Farm Address:</span>
                      <strong className="text-slate-800 font-bold leading-tight">{selectedMember.farmAddress || 'SADC Agrarian Grid'}</strong>
                    </div>
                  </>
                )}
                ...
                {selectedMember.userRole === 'Transporter' && (
                  <>
                    <div className="text-xs">
                      <span className="text-slate-400 font-medium block">Carrier Logistics Base:</span>
                      <strong className="text-indigo-900 font-bold">{selectedMember.location || 'Beitbridge Base Hub'}</strong>
                    </div>
                    <div className="text-xs">
                      <span className="text-slate-400 font-medium block">Haulage Fleet Capacity:</span>
                      <strong className="text-slate-800 font-bold">10-22 Tonne Flatbed Sacks</strong>
                    </div>
                  </>
                )}
                ...
                {selectedMember.userRole === 'Dealer' && (
                  <>
                    <div className="text-xs">
                      <span className="text-slate-400 font-medium block">Target Crops Procuring:</span>
                      <strong className="text-amber-800 font-semibold">{selectedMember.cropLookingFor || 'Maize, Wheat'}</strong>
                    </div>
                    <div className="text-xs">
                      <span className="text-slate-400 font-medium block">Depot Trading Floor:</span>
                      <strong className="text-slate-800 font-bold">{selectedMember.location || 'Mbare Commercial Markets'}</strong>
                    </div>
                  </>
                )}
                ...
                <div className="pt-2 border-t border-slate-100 space-y-2">
                  <h4 className="text-[10px] uppercase font-bold text-slate-400">Interconnect Contact Links</h4>
                  ...
                  <div className="space-y-1.5">
                    ...
                    >
                      <span className="flex items-center gap-1.5"><Phone size={13} className="text-slate-400" /> {selectedMember.phoneNumber}</span>
                    ...
                    {selectedMember.email && (
                      <a 
                        ...
                        className="w-full bg-slate-50 border border-slate-200 hover:bg-slate-100 font-bold text-xs p-2.5 rounded-xl flex items-center justify-between text-slate-700"
                      >
                        <span className="flex items-center gap-1.5"><Mail size={13} className="text-slate-400" /> {selectedMember.email}</span>
                ...
                <div className="mt-4 p-3 rounded-xl border border-rose-100 bg-rose-50/50 flex gap-2 text-[10px] text-rose-800 leading-normal">
                  ...
                  <span>
                    <strong>Security Clearance Restrict:</strong> To prevent fraud, third parties can inspect credentials only. Editing is blocked without local signature keys matching account #{selectedMember.id}.
                  </span>
                </div>
              </div>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6 text-center text-xs text-slate-450 flex flex-col items-center justify-center min-h-[300px] space-y-2">
              ...
              <span>Tap on any member profile on the left registry list to view their verification parameters, fleet base bases, specialties, and contact details prominently.</span>
            </div>
          )}
        </div>
