...
export default function TransporterView({
  ...
  const uploadedDocs =
    useLiveQuery(() =>
      db.kycDocuments.where("userId").equals(user.id!).toArray(),
    ) || [];
  ...
  const getDocDisplayName = (type: string) => {
    switch (type) {
      ...
      case "VehicleReg":
        return "Vehicle Registration Book";
      case "VehiclePhoto":
        return "Fleet Vehicle Photo";
      default:
        return type;
    }
  };
  ...
  const hasAllDocs = requiredDocTypes.every((type) =>
    uploadedDocs.some((d) => d.docType === type),
  );
      ...
      <div className="space-y-6">
        <header className="mb-6">
          <h1 className="text-2xl font-bold font-sans text-slate-900 tracking-tight">
            Transporter Verification Required
          </h1>
          <p className="text-sm text-slate-500 mt-1">
            Submit regional KYC identity items to access AgriPulse logistical
            cargoes.
          </p>
        </header>
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <h2 className="text-base font-bold text-slate-800">
                SADC Interstate Carrier Verification
              </h2>
              <p className="text-xs text-slate-500 font-mono mt-0.5">
                Status: Pending Document Vault
              </p>
            </div>
          </div>
          <p className="text-xs text-slate-600 mb-6 leading-relaxed">
            Upload active documentation. Our localized canvas engine
            auto-compresses assets before replication. Bids are locked until
            SADC identity check completes.
          </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {requiredDocTypes.map((docType) => {
              ...
              return (
                <div
                  key={docType}
                  className={`border-2 rounded-xl p-4 flex flex-col justify-between h-40 transition-all relative overflow-hidden bg-slate-50 border-slate-200 hover:border-emerald-500 ${savedDoc ? "border-emerald-100 bg-emerald-50/10" : ""}`}
                >
                  {savedDoc && (
                    <img
                  ...
                  <div className="mt-4">
                    {isUploading ? (
                      <span className="text-xs text-emerald-600 font-semibold font-mono animate-pulse">
                        Compressing local file...
                      </span>
                    ) : savedDoc ? (
                      <div className="flex items-center justify-between mt-1">
                        <span className="text-[10px] text-slate-400 font-mono">
                          Size: ~
                          {(savedDoc.fileDataUrl.length / 1024).toFixed(1)} KB
                        </span>
                        <label className="text-[10px] font-bold text-emerald-700 hover:text-emerald-800 uppercase cursor-pointer">
                          ...
                          <input
                            ...
                            accept="image/*"
                            className="hidden"
                            onChange={(e) => handleKYCUpload(e, docType)}
                          />
                        </label>
                      </div>
                    ) : (
                      <label className="w-full flex items-center justify-center gap-1.5 border border-dashed border-slate-350 hover:border-emerald-500 hover:bg-white text-slate-600 font-semibold py-2.5 px-3 rounded-lg text-xs cursor-pointer transition-colors">
                        <Camera size={14} className="text-slate-405" /> Snap
                        Document
                        <input
                          type="file"
                          accept="image/*"
          ...
          <div className="mt-8 border-t border-slate-100 pt-5 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-2 text-slate-500">
              ...
              <span className="text-xs font-semibold">
                {uploadedDocs.length} of {requiredDocTypes.length} documents
                uploaded.
              </span>
            </div>
            <button
    ...
    <div className="space-y-6">
      ...
      {activeTab === "dashboard" && (
        <div className="space-y-6">
          <header className="mb-4 flex justify-between items-start">
            <div>
              <h1 className="text-2xl font-bold text-slate-800 tracking-tight">
                Alliance Logistics Feed
              </h1>
              <p className="text-xs text-slate-500 font-mono mt-0.5">
                Active farm listings, dealer requests, and courier classifieds.
              </p>
            </div>
            <button
          ...
          <div className="flex justify-between items-center bg-white p-4 rounded-2xl border border-slate-200 shadow-3xs">
            <div className="text-left">
              <h4 className="text-xs font-bold text-slate-800">
                Propose Transport Services
              </h4>
              <p className="text-[10px] text-slate-400 mt-0.5">
                Advertise your carriage specs and shipping rates to farmers &
                dealers.
              </p>
            </div>
            <button
          ...
          {showServiceForm && (
            <form
              onSubmit={handlePostServiceOffer}
            ...
            >
              <h3 className="font-bold text-xs text-slate-700 uppercase">
                Publish Haulage Service Advertisement
              </h3>
              ...
              <div className="space-y-1">
                ...
                <input
                  required
                  placeholder="e.g. 15-Tonne Flatbed Carriage: Mutare to Harare"
                  value={srvTitle}
              ...
              <div className="space-y-1">
                <label className="text-[9px] font-bold text-slate-500 uppercase">
                  Standard Rate (USD / Tonne or Trip)
                </label>
                <input
                  required
                  type="number"
              ...
              <div className="space-y-1">
                ...
                <textarea
                  required
                  placeholder="Available transit days, volume specifications, truck tare weights, dropoff areas..."
                  value={srvDesc}
                  onChange={(e) => setSrvDesc(e.target.value)}
                  className="w-full bg-white border border-slate-200 p-2 text-xs rounded-lg outline-none min-h-[60px]"
                />
              </div>

              <button
                type="submit"
                className="w-full bg-indigo-600 hover:bg-indigo-750 text-white font-bold text-xs py-2 rounded-xl"
              >
          ...
          <section className="space-y-3">
            ...
            {globalAdverts.filter((a) => a.authorId === user.id).length === 0 ? (
              <div className="bg-white p-6 rounded-2xl border border-slate-200 text-center text-xs text-slate-400">
                You have not posted any trucking service ads yet.
              </div>
            ) : (
                      ...
                      >
                        ...
                        <div className="p-4 bg-slate-50 space-y-2">
                          <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest pl-1 block">
                            Service Queries & Offers ({advBids.length})
                          </span>
                          {advBids.length === 0 ? (
          ...
          <section className="space-y-4">
            <h2 className="text-xs font-bold text-slate-500 uppercase tracking-widest flex items-center gap-1">
              <Truck size={14} className="text-indigo-600" /> Active Farm Cargo
            ...
            {activeRequests.length === 0 ? (
              ...
              <div className="space-y-3">
                {activeRequests.map((req) => {
                  const alreadyBidded = myBids.some(
                    (b) => b.requestId === req.id,
                  );
                  return (
                    <div
                      key={req.id}
                      className="bg-white rounded-2xl p-4 border border-slate-200 shadow-3xs text-left relative overflow-hidden"
                    >
                      {alreadyBidded && (
          ...
          <section className="space-y-4 pt-4 border-t border-slate-150">
            <h2 className="text-xs font-bold text-slate-500 uppercase tracking-widest flex items-center gap-1.5All flex-wrap">
              <Megaphone size={14} className="text-indigo-650" /> SADC Alliance
              community classifieds ({globalAdverts.length})
            </h2>

            {globalAdverts.length === 0 ? (
              <p className="text-xs text-slate-450 italic">
                No community adverts registered in global index.
              </p>
            ) : (
              <div className="space-y-4">
                {globalAdverts.map((adv) => (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        <span
                          className={`text-[8px] font-bold px-2 py-0.5 rounded uppercase ${adv.type === "ProduceSale" ? "bg-emerald-100 text-emerald-700" : adv.type === "TransportOffer" ? "bg-indigo-50 text-indigo-700" : "bg-amber-100 text-amber-700"}`}
                        >
      ...
      {activeTab === "activity" && (
        <div className="space-y-6">
          <header className="mb-4">
            <h1 className="text-2xl font-bold text-slate-800 tracking-tight">
              Active Transits & Bids
            </h1>
            <p className="text-xs text-slate-500 font-mono mt-0.5">
              Track cargo tenders, contract locks, and negotiations.
            </p>
          </header>

          <section className="space-y-4">
            <h2 className="text-xs font-bold text-slate-500 uppercase tracking-widest">
              Bids Placed Locally ({myBids.length})
            </h2>
            ...
            {myBids.length === 0 ? (
              <div className="bg-white p-8 rounded-2xl border border-slate-200 text-center text-xs text-slate-400 flex flex-col items-center justify-center space-y-4">
                <span>
                  You have not placed any bids or counter rates yet. Go to Feed
                  to get started.
                </span>
                <button
                  onClick={() =>
                    alert(
                      "Please tap on 'Logistics Dashboard' in your left sidebar to view active cargo feed listings.",
                    )
                  }
                  className="bg-indigo-650 hover:bg-indigo-700 text-white font-black text-[10px] uppercase tracking-wider px-4 py-2 rounded-xl transition-all shadow-3xs cursor-pointer"
                >
              ...
              <div className="space-y-3">
                {myBids.map((bid) => {
                  ...
                  return (
                    <div
                      key={bid.id}
                      className="bg-white border border-slate-200 p-4 rounded-2xl shadow-3xs text-left"
                    >
                      <div className="flex justify-between items-center">
                        <div>
                          ...
                          <p className="font-bold text-slate-800 text-xs sm:text-sm mt-1.5">
                            Submitted Counter rate:{" "}
                        ...
                        <span
                          className={`text-[9px] font-extrabold px-2.5 py-1 rounded shadow-3xs uppercase ${bid.status === "Accepted" ? "bg-emerald-100 text-emerald-800" : bid.status === "Rejected" ? "bg-rose-100 text-rose-800" : "bg-amber-100 text-amber-800"}`}
                        >
                          {bid.status}
                        </span>
                      </div>
                        ...
                        <div className="mt-3 bg-emerald-50 border border-emerald-100 p-3 rounded-xl">
                          ...
                          <div className="flex gap-2 p-2 bg-amber-50 border border-amber-100 rounded-lg text-amber-800 text-[9px] leading-relaxed font-medium">
                            ...
                            <p>
                              ...
                              deploying fleet. The app developers are not
      ...
      {activeTab === "profile" && (
        <div className="space-y-6">
          <header className="mb-4">
            ...
            <p className="text-xs text-slate-500 font-mono mt-0.5">
              Configure driver settings, view fleet status, and active KYC.
            </p>
          </header>

          <section className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
            <h2 className="text-sm font-bold text-slate-800 mb-4 flex items-center gap-1.5">
              <Settings size={16} className="text-indigo-600" /> Adjust Profile
              Settings
            </h2>
            ...
            >
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 focus:border-indigo-505 rounded-xl p-2.5 text-xs outline-none"
                />
              </div>
          ...
          <section className="bg-white p-5 rounded-2xl border border-slate-200">
            <h2 className="text-sm font-bold text-slate-800 mb-2">
              Secure SADC ID Tokens
            </h2>
            <p className="text-xs text-slate-450 leading-relaxed mb-4">
              Your four verification certificates are securely encrypted in your
              browser replica cache.
            </p>
            ...
            <div className="grid grid-cols-2 gap-3">
              {requiredDocTypes.map((docType) => {
                ...
                return (
                  <div
                    key={docType}
                    className="bg-slate-50 border border-slate-150 p-2.5 rounded-xl flex items-center justify-between text-left"
                  >
                    ...
                    <span
                      className={`w-2.5 h-2.5 rounded-full ${doc ? "bg-emerald-500" : "bg-slate-200"}`}
                    ></span>
                  </div>
                );
              })}
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl relative text-left">
            <button
              ...
              className="absolute top-4 right-4 text-slate-400 hover:text-slate-750"
            >
              <X size={18} />
            </button>
            <h3 className="text-base font-bold flex items-center mb-1">
              <Gavel size={18} className="mr-2 text-indigo-600" /> Submit
              Counter rate
            </h3>
            <p className="text-xs text-slate-500 mb-5">
              Your offer will pin instantly onto {bidModalReq.farmerName}'s
              request dashboard.
            </p>

            <form onSubmit={handlePlaceBid}>
              <div className="mb-4">
                <label className="text-[10px] font-semibold text-slate-505 uppercase tracking-wider mb-1 block">
                  Offer Rate (USD)
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-350 rounded-xl py-2.5 pl-9 pr-4 text-sm font-bold outline-none focus:border-indigo-500 focus:bg-white"
                  />
                </div>
              </div>
              <button
                type="submit"
                className="w-full bg-indigo-650 hover:bg-indigo-700 text-white font-bold text-xs py-3 rounded-xl shadow-md transition-all"
              >
                Submit Freight Bid
              </button>
            </form>
