...
import { 
  PlusCircle, Leaf, Scale, DollarSign, MapPin, Camera, X, Check, ArrowRight,
  ShieldCheck, Phone, Mail, User as UserIcon, Settings, Tag, Users, AlertCircle,
  Megaphone, MessageSquare, Truck, Store, Share2
} from 'lucide-react';
import { syncManager, SyncLog } from '../syncManager';

export default function FarmerView({ user, activeTab }: { user: User; activeTab: 'dashboard' | 'activity' | 'profile' }) {
  ...
  const [editSpecialties, setEditSpecialties] = useState(user.cropSpecializations || '');
  ...
  const myRequests = useLiveQuery(() => db.transportRequests.where('farmerId').equals(user.id!).reverse().sortBy('createdAt')) || [];
  const allUsers = useLiveQuery(() => db.users.toArray()) || [];
  const globalAdverts = useLiveQuery(() => db.adverts.toArray()) || [];
  const allBids = useLiveQuery(() => db.bids.toArray()) || [];

  const handleMediaCapture = (e: React.ChangeEvent<HTMLInputElement>, target: 'request' | 'advert' | 'avatar') => {
  ...
  const handleAcceptBid = async (bid: Bid) => {
    await dataAccess.transportRequests.update(bid.requestId, { status: 'InProgress' });
    await dataAccess.bids.update(bid.id!, { status: 'Accepted' });
    
    // Reject other competing bids
    const peerBids = allBids.filter(b => b.requestId === bid.requestId && b.id !== bid.id);
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'dashboard' && (
        <div className="space-y-6">
          <header className="mb-4">
            <h1 className="text-2xl font-bold text-slate-800 tracking-tight">Farmer Logs & Cargo Bids</h1>
            <p className="text-xs text-slate-500 font-mono mt-0.5">Publish harvest sacks and match haulage transport.</p>
          </header>
          ...
          <section className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-sm font-bold text-slate-800 uppercase tracking-wider">New Transport Request Form</h2>
            </div>

            <form id="farmer-request-form" onSubmit={handleRequestSubmit} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Crop Variety</label>
                  <select 
                    value={crop} 
                ...
                {crop === 'Other' && (
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Specify Crop Name</label>
                    <input 
                      required 
                      type="text" 
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Harvest Yield Weight</label>
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Farm Origin Address</label>
                  <input 
                    required 
                    type="text" 
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Destination Market / Depot</label>
                  <input 
                    required 
                    type="text" 
                ...
                <div className="space-y-1 sm:col-span-2">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Expected Courier Pricing (USD)</label>
                ...
                <div className="space-y-1 sm:col-span-2">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Attach Optional Crop Photo Preview</label>
                  <div className="border-2 border-dashed border-slate-200 hover:border-emerald-500 relative cursor-pointer overflow-hidden rounded-xl h-28 flex flex-col items-center justify-center bg-slate-50 text-slate-400">
                    {mediaPreview ? (
                      <div className="relative w-full h-full">
                        <img src={mediaPreview} alt="Yield Snap" className="w-full h-full object-cover" />
                        <button type="button" onClick={() => setMediaPreview(null)} className="absolute top-2 right-2 p-1 rounded-full bg-slate-900/80 text-white hover:bg-slate-950">
                          <X size={14} />
                        </button>
                      </div>
                    ) : (
                      <>
                        ...
                        <span className="text-[10px] font-semibold text-slate-500">Pick or Snapshot Crop Media</span>
                        <span className="text-[8px] uppercase font-mono tracking-widest text-slate-450 mt-1">Processed offline</span>
                      </>
                    )}
                    <input type="file" accept="image/*" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" onChange={(e) => handleMediaCapture(e, 'request')} />
                  </div>
                </div>

              </div>

              <button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 rounded-xl shadow-md transition-colors text-xs uppercase tracking-wider">
          ...
          <section className="space-y-3">
            <h2 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Your Open Freight Jobs</h2>
            {myRequests.length === 0 ? (
              <div className="bg-white p-8 rounded-2xl border border-slate-200 text-center text-xs text-slate-400 flex flex-col items-center justify-center space-y-4">
                <span>You have not initiated any transport requests yet. Complete the form above to deploy.</span>
                <button
                  onClick={() => {
                    const targetEl = document.getElementById('farmer-request-form');
              ...
              <div className="space-y-4">
                {myRequests.map(req => {
                  const reqBids = allBids.filter(b => b.requestId === req.id && b.status === 'Pending');
                  return (
                    <div key={req.id} className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                      <div className="p-4 flex flex-col sm:flex-row justify-between gap-4 border-b border-slate-100">
                        <div className="flex gap-3">
                          {req.image && (
                            <img src={req.image} alt="Crop" className="w-14 h-14 rounded-xl object-cover border border-slate-150 flex-shrink-0" />
                          )}
                          <div>
                            ...
                            <h3 className="font-bold text-slate-800 text-sm mt-1">{req.quantity} {req.unit}</h3>
                            <p className="text-[11px] text-slate-400 flex items-center gap-1 mt-0.5 font-medium">
                              <MapPin size={11} /> {req.origin} → {req.destination}
                            </p>
                          </div>
                        </div>
                        <div className="text-left sm:text-right flex flex-col justify-between">
                          <span className={`text-[9px] w-fit sm:self-end font-bold px-2 py-0.5 rounded shadow-3xs uppercase ${req.status === 'Open' ? 'bg-amber-100 text-amber-800' : 'bg-indigo-150 text-indigo-800'}`}>
                            {req.status === 'Open' ? 'Awaiting Bids' : req.status === 'InProgress' ? 'In Transit / Dispatched' : 'Completed'}
                          </span>
                          <p className="font-bold text-slate-700 text-xs mt-1">Expected: <span className="text-emerald-600 font-extrabold">${req.targetPrice}</span></p>
                        </div>
                      </div>
                      ...
                      <div className="p-4 bg-slate-50 space-y-2">
                        ...
                        {reqBids.length === 0 ? (
                          <p className="text-[10px] text-slate-450 italic pl-1">No bids submitted yet by regional carriers...</p>
                        ) : (
      ...
      {activeTab === 'activity' && (
        <div className="space-y-6">
          <header className="mb-4">
            <h1 className="text-2xl font-bold text-slate-800 tracking-tight">Mesh Grid Activities</h1>
            <p className="text-xs text-slate-500 font-mono mt-0.5">Dispatched carrier actions, other local farmers, and buy targets.</p>
          </header>
          ...
          <section className="space-y-4">
            <div className="flex items-center gap-2 border-b border-slate-150 pb-2">
              ...
              <h2 className="text-xs font-extrabold text-slate-500 uppercase tracking-widest">Active AgriPulse SADC Alliance directory ({allUsers.length})</h2>
            </div>

            <div className="grid grid-cols-1 gap-3">
              {allUsers.map((item) => (
                <div key={item.id} className="bg-white rounded-2xl border border-slate-200 p-4 shadow-3xs flex items-start gap-3">
                  <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center border border-slate-150 overflow-hidden flex-shrink-0">
                    {item.profileImage ? (
                      ...
                      <span className="font-bold text-slate-600 text-sm">
                        {item.userRole === 'Farmer' ? '🌾' : item.userRole === 'Transporter' ? '🚚' : '🏬'}
                      </span>
                    )}
                  </div>
                  <div className="flex-1 text-left min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className={`text-[8px] font-bold px-1.5 py-0.5 rounded-full uppercase tracking-wider ${item.userRole === 'Farmer' ? 'bg-emerald-50 text-emerald-700' : item.userRole === 'Transporter' ? 'bg-indigo-50 text-indigo-700' : 'bg-amber-50 text-amber-700'}`}>
                        {item.userRole}
                      </span>
                    </div>
                    
                    <p className="text-[10px] text-slate-450 mt-1 flex items-center gap-1">
                      <MapPin size={11} className="text-slate-400" /> {item.farmAddress || item.location || 'Coordinating hub location'}
                    </p>
                    
                    {item.userRole === 'Farmer' && item.cropSpecializations && (
                      <p className="text-[10px] text-slate-550 mt-1 italic">Speaks: {item.cropSpecializations}</p>
                    )}
                    {item.userRole === 'Dealer' && item.cropLookingFor && (
                      <p className="text-[10px] text-slate-550 mt-1 italic">Sourcing: {item.cropLookingFor}</p>
                    )}

                    <div className="flex gap-4 mt-2.5 pt-2.5 border-t border-slate-100 text-[10px] font-semibold text-slate-500">
                      <span className="flex items-center gap-1"><Phone size={10} className="text-slate-400" /> {item.phoneNumber}</span>
                      {item.email && <span className="flex items-center gap-1"><Mail size={10} className="text-slate-400" /> {item.email}</span>}
                    </div>
                  </div>
                </div>
              ))}
          ...
          <section className="space-y-4">
            <div className="flex justify-between items-center border-b border-slate-150 pb-2">
              <span className="flex items-center gap-1.5 font-bold text-xs text-slate-550 uppercase tracking-wider">
                <Megaphone size={16} className="text-emerald-600" /> Interactive Exchange Feed ({globalAdverts.length})
              </span>
              <button 
            ...
            {showAdvertForm && (
              <form onSubmit={handleCreateAdvert} className="bg-slate-50 rounded-2xl border border-slate-150 p-4 space-y-3">
                <h3 className="font-bold text-xs text-slate-700 uppercase">Publish Direct Smallholder Produce Ad</h3>
                
                <div className="space-y-1">
                  <label className="text-[9px] text-slate-500 font-bold uppercase tracking-wider">Ad Title</label>
                ...
                <div className="space-y-1">
                  ...
                  <textarea required placeholder="Yield metrics, baggage condition, bulk bags loaded..." value={advDesc} onChange={e => setAdvDesc(e.target.value)} className="w-full bg-white border border-slate-200 rounded-lg p-2 text-xs outline-none min-h-[50px]" />
                </div>
                ...
                <button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs py-2 rounded-xl transition-all">
            ...
            {globalAdverts.length === 0 ? (
              <div className="bg-white p-6 rounded-2xl border border-slate-200 text-center text-xs text-slate-400">
                Peer Classified Exchange is ready. SADC alliances have not posted produce ads today.
              </div>
            ) : (
              <div className="space-y-4">
                {globalAdverts.map((adv) => (
                  <div key={adv.id} className="bg-white rounded-2xl border border-slate-200 p-4 shadow-3xs">
                    <div className="flex justify-between items-start gap-4">
                      <div>
                        <span className={`text-[8px] font-bold px-2 py-0.5 rounded uppercase tracking-wider ${adv.type === 'ProduceSale' ? 'bg-emerald-100 text-emerald-800' : adv.type === 'TransportOffer' ? 'bg-indigo-150 text-indigo-800' : 'bg-amber-100 text-amber-800'}`}>
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-[8px] uppercase font-mono text-slate-400 font-bold">{adv.authorRole}</p>
                      </div>
                    </div>
      ...
      {activeTab === 'profile' && (
        <div className="space-y-6">
          <header className="mb-4">
            <h1 className="text-2xl font-bold text-slate-800 tracking-tight">Farm Coordinates & Settings</h1>
            <p className="text-xs text-slate-500 font-mono mt-0.5">Update smallholder data and offline synchronization diagnostics.</p>
          </header>

          <section className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
            <h2 className="text-sm font-bold text-slate-800 tracking-tight mb-4 flex items-center gap-1.5">
              <Settings size={16} className="text-emerald-600" /> Account Settings
            </h2>
            ...
            {saveSuccess && (
              <div className="bg-emerald-50 border border-emerald-100 text-emerald-800 text-xs font-semibold p-3 rounded-xl mb-4 flex items-center gap-2">
                <Check size={16} /> Update Saved Locally! Synchronization broadcast launched.
              </div>
            )}
            ...
            <form onSubmit={handleUpdateProfile} className="space-y-4">
              <div className="flex items-center gap-4 border-b border-slate-100 pb-4">
                <div className="w-14 h-14 rounded-full bg-slate-100 border border-slate-200 relative overflow-hidden flex items-center justify-center flex-shrink-0">
                  {editAvatar ? (
                    <img src={editAvatar} alt="Farm profile" className="w-full h-full object-cover" />
                  ) : (
                    <UserIcon size={24} className="text-slate-400" />
                  )}
                  <input type="file" accept="image/*" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" onChange={(e) => handleMediaCapture(e, 'avatar')} />
                </div>
                <div>
                  <h3 className="font-bold text-slate-800 text-sm">Avatar / Crop Branding</h3>
                  <p className="text-[10px] text-slate-400 mt-0.5">Tap on picture bubble to snapshot smallholder photo.</p>
                </div>
              </div>

              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Farmers Name</label>
                <input required type="text" value={editName} onChange={e => setEditName(e.target.value)} className="w-full bg-slate-50 border border-slate-200 focus:border-emerald-500 rounded-xl p-2.5 text-xs outline-none" />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Contacts Phone</label>
                  <input required type="text" value={editPhone} onChange={e => setEditPhone(e.target.value)} className="w-full bg-slate-50 border border-slate-200 focus:border-emerald-500 rounded-xl p-2.5 text-xs outline-none" />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Contacts Email</label>
                  <input required type="email" value={editEmail} onChange={e => setEditEmail(e.target.value)} className="w-full bg-slate-50 border border-slate-200 focus:border-emerald-500 rounded-xl p-2.5 text-xs outline-none" />
                </div>
              </div>

              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Farm address & Location</label>
                <input required type="text" value={editLocation} onChange={e => setEditLocation(e.target.value)} className="w-full bg-slate-50 border border-slate-200 focus:border-emerald-500 rounded-xl p-2.5 text-xs outline-none" />
              </div>

              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Crop Specializations</label>
                <input required type="text" value={editSpecialties} onChange={e => setEditSpecialties(e.target.value)} className="w-full bg-slate-50 border border-slate-200 focus:border-emerald-500 rounded-xl p-2.5 text-xs outline-none" />
              </div>

              <button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2.5 rounded-xl text-xs transition-colors shadow-sm">
          ...
          <section className="bg-white p-5 rounded-2xl border border-slate-200">
            <h2 className="text-sm font-bold text-slate-800 tracking-tight mb-3">Sync Diagnostic Ledger</h2>
            <div className="bg-slate-50 border border-slate-150 rounded-xl p-3 grid grid-cols-2 gap-4 text-center">
              <div>
                ...
                <p className="text-lg font-black text-slate-800 mt-1">{pendingSyncCount} items waiting</p>
              </div>
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Verification Status</p>
                <p className="text-lg font-black text-emerald-600 mt-1">{user.verificationStatus}</p>
              </div>
            </div>

            <div className="mt-4 space-y-2 max-h-48 overflow-y-auto pr-1">
              {syncLogs.slice(0, 8).map(log => (
                <div key={log.id} className="flex justify-between items-center text-[10px] font-mono border border-slate-100 p-2 rounded-lg bg-slate-50">
                  <span className="truncate text-slate-600">{log.description}</span>
                  <span className={`font-bold px-1.5 py-0.5 rounded text-[8px] tracking-wide ${log.status === 'success' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-200 text-slate-500'}`}>
