...
import farmerImg from "./assets/images/high_contrast_farmer_1781696713011.jpg";
import truckImg from "./assets/images/high_contrast_truck_1781696729960.jpg";
import marketImg from "./assets/images/high_contrast_market_1781696745154.jpg";
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<
    "dashboard" | "activity" | "members" | "profile"
  >("dashboard");
  ...
  const [cropSpecializations, setCropSpecializations] =
    useState("Maize, Wheat");
  ...
  useEffect(() => {
    ...
    // Setup native notification listeners via Service Worker
    async function setupNotifications() {
      if ("Notification" in window && navigator.serviceWorker) {
        ...
        const rdb = await db.getRawInstance();

        // Listen to new inserts on transportRequests
        const reqSub = rdb.transportrequests.$.subscribe((changeEvent: any) => {
          if (changeEvent.operation === 'INSERT' && Notification.permission === 'granted') {
            new Notification('New Transport Request', {
               body: `A new cargo of ${changeEvent.documentData.cropName} from ${changeEvent.documentData.origin} was posted.`,
               icon: '/agripulse_logistics_icon.jpg'
            });
          }
        });

        // Listen to new inserts on adverts
        const advSub = rdb.adverts.$.subscribe((changeEvent: any) => {
          if (changeEvent.operation === 'INSERT' && Notification.permission === 'granted') {
            new Notification('New Market Advert', {
               body: `${changeEvent.documentData.title} by ${changeEvent.documentData.authorName}.`,
               icon: '/agripulse_logistics_icon.jpg'
            });
          }
        });

        // Listen to bid changes (new or accepted)
        const bidSub = rdb.bids.$.subscribe((changeEvent: any) => {
           if (Notification.permission === 'granted') {
              if (changeEvent.operation === 'INSERT') {
                 new Notification('New Local Bid Activity', {
                    body: `Bid of $${changeEvent.documentData.offerPrice} added by ${changeEvent.documentData.bidderName}.`,
                    icon: '/agripulse_logistics_icon.jpg'
                 });
              } else if (changeEvent.operation === 'UPDATE' && changeEvent.documentData.status === 'Accepted') {
                 new Notification('Bid Accepted', {
                    body: `A bid of $${changeEvent.documentData.offerPrice} has been accepted!`,
                    icon: '/agripulse_logistics_icon.jpg'
                 });
              }
           }
  ...
  useEffect(() => {
    const fetchTickerData = async () => {
      try {
        ...
        reqs.forEach((r) =>
          items.push(
            `🌾 Cargo Offer: ${r.quantity} ${r.unit} of ${r.cropName} from ${r.origin} to ${r.destination} proposed budget $${r.targetPrice}`,
          ),
        );
        bidsList.forEach((b) =>
          items.push(
            `🚚 Carrier bid proposed: Rate of $${b.offerPrice} submitted by user ${b.bidderName}`,
          ),
        );
        ads.forEach((a) =>
          items.push(
            `📢 SADC agri Bulletin: ${a.title} posted by ${a.authorName} - Market rate $${a.price}`,
          ),
        );
        ...
        if (items.length === 0) {
          items.push(
            "🌾 Cargo Load Request: 15 Tonnes of yellow maize from Mazowe to Harare Seek Carrier",
          );
          items.push(
            "🚚 Active Driver Freight Bid: $420 USD bid placed for Harare Wheat run",
          );
          items.push(
            "📢 SADC Trade Market Call: Grain procurement tender active in Mbare Hub",
          );
          items.push(
            "🏬 Wholesale Buy Tender: Maize 40 Tonnes needed under Harare Agency",
          );
        }
  ...
  const handleProfileImageCapture = (
  ...
  const validatePhoneAndPin = (phone: string, pin: string) => {
    ...
    if (!phoneRegex.test(phone)) {
      alert(
        "Invalid phone number format. Please use standard format, e.g. +263771234567",
      );
      return false;
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans relative pt-14 pb-20">
      ...
      {viewState === "landing" && (
        <div className="flex-grow flex flex-col" id="landing-page-component">
          ...
          <div className="bg-slate-900 text-white pt-16 pb-20 px-6 text-center relative overflow-hidden border-b border-emerald-500/20">
            ...
            <div className="max-w-4xl mx-auto relative z-10">
              <span className="inline-flex items-center gap-1.5 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs px-3 py-1 rounded-full font-semibold uppercase tracking-wider mb-5">
                <Globe size={13} className="animate-spin-slow" /> Regional
                Agriculture & Logistical Network
              </span>
              <h1 className="text-4xl sm:text-5xl font-extrabold font-sans leading-tight tracking-tight text-white mb-4">
                ...
                <span className="text-emerald-400 bg-emerald-950/40 px-2 rounded-lg">
                  Transporters & Market Dealers
                </span>
              </h1>
              <p className="text-sm sm:text-base text-slate-300 max-w-2xl mx-auto mb-8 font-medium">
                AgriPulse Connect is an offline-resilient peer-to-peer directory
                and logistics exchange enabling transparent pricing, verified
                cargo manifests, and rapid trade bidding.
              </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden group hover:shadow-xl transition-all flex flex-col">
                ...
                <div className="p-5 flex-grow flex flex-col justify-between">
                  <div>
                    <h3 className="font-bold text-slate-850 text-base mb-2">
                      Publish Harvests Locally
                    </h3>
                    <p className="text-xs text-slate-500 leading-relaxed">
                      Farmers register yield catalogs and submit direct
                      logistical pickup requests while offline. Crops are
                      geo-indexed so nearby cargo haulers discover them
                      instantly.
                    </p>
                  </div>
                  <div className="mt-4 pt-3 border-t border-slate-100 flex items-center gap-1.5 text-xs text-emerald-600 font-semibold">
                    <span>1. Crop Specialization</span> •{" "}
                    <span>Bulk Sacks</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden group hover:shadow-xl transition-all flex flex-col">
                ...
                <div className="p-5 flex-grow flex flex-col justify-between">
                  <div>
                    <h3 className="font-bold text-slate-850 text-base mb-2">
                      Bid & Propose Empty Runs
                    </h3>
                    <p className="text-xs text-slate-500 leading-relaxed">
                      Carriers review active harvest requests on the SADC feed
                      and pitch transport rates. Secure localized KYC
                      credentials protect both drivers and yield owners.
                    </p>
                  </div>
                  <div className="mt-4 pt-3 border-t border-slate-100 flex items-center gap-1.5 text-xs text-blue-600 font-semibold">
                    <span>2. KYC Documents Secure</span> •{" "}
              ...
              <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden group hover:shadow-xl transition-all flex flex-col">
                ...
                <div className="p-5 flex-grow flex flex-col justify-between">
                  <div>
                    <h3 className="font-bold text-slate-850 text-base mb-2">
                      Buy Direct & List Sourcing
                    </h3>
                    <p className="text-xs text-slate-500 leading-relaxed">
                      Wholesale agro-dealers buy whole yields directly, hire
                      transporters, and advertise target crops, ensuring
                      smallholders receive real value.
                    </p>
                  </div>
                  <div className="mt-4 pt-3 border-t border-slate-100 flex items-center gap-1.5 text-xs text-amber-600 font-semibold">
                    <span>3. Retail Distribution</span> •{" "}
                    <span>Stable Sourcing</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="max-w-4xl mx-auto">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-xs sm:text-sm">
                <div className="flex gap-3">
                  <div className="w-8 h-8 rounded-lg bg-emerald-100 text-emerald-600 flex items-center justify-center font-bold flex-shrink-0">
                    ✓
                  </div>
                  <div>
                    <h4 className="font-bold text-slate-800">
                      100% Offline-Resilient Synchronization
                    </h4>
                    <p className="text-slate-500 mt-1">
                      Peer-to-peer localized browser databases hold your bids.
                      Transactions automatically update when network signals
                      return.
                    </p>
                  </div>
                </div>
                <div className="flex gap-3">
                  ...
                  <div>
                    <h4 className="font-bold text-slate-800">
                      Zero Commission Peer Trading
                    </h4>
                    <p className="text-slate-500 mt-1">
                      Direct negotation between farms and dealers without
                      middleman fees. Farmers can review and pick any bid they
                      trust.
                    </p>
                  </div>
                </div>
                <div className="flex gap-3">
                  ...
                  <div>
                    <h4 className="font-bold text-slate-800">
                      Strict Driver Identification Standards
                    </h4>
                    <p className="text-slate-500 mt-1">
                      Transporters undergo a locally cached four-item KYC
                      verification book, creating complete accountability.
                    </p>
                  </div>
                </div>
                <div className="flex gap-3">
                  ...
                  <div>
                    <h4 className="font-bold text-slate-800">
                      Global SADC Interactive Feed
                    </h4>
                    <p className="text-slate-500 mt-1">
                      Interactive community stream allows sellers, carriers, and
                      wholesale hubs to broadcast classified ads, list
                      capabilities, and publish prices.
                    </p>
                  </div>
                </div>
              </div>
          ...
          <footer className="text-center py-8 text-xs text-slate-400 font-mono">
            AgriPulse SADC Logistical Alliance Hub • Running locally on client
            replication engines
          </footer>
        </div>
      )}
      ...
      {viewState === "choose_role" && (
        <div className="max-w-md mx-auto w-full px-6 py-12 flex-grow flex flex-col justify-center relative">
          <button
            onClick={() => setViewState("landing")}
            className="absolute top-0 left-6 text-slate-500 hover:text-slate-800 text-xs font-bold flex items-center gap-1 bg-slate-100 px-3 py-1.5 rounded-lg transition-colors"
          >
            ← Back to Information
          </button>
          <div className="text-center mb-6 mt-8">
            <h2 className="text-2xl font-bold text-slate-900 tracking-tight">
              Agricultural Alliance Hub
            </h2>
            <p className="text-xs text-slate-500 mt-2">
              Manage your peer-to-peer SADC trading and logistical account.
            </p>
          </div>
          ...
          {authTab === "register" ? (
            <div className="space-y-4">
              <p className="text-[11px] text-slate-500 text-center mb-2">
                Choose your business model below to start your localized
                profile.
              </p>
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    Upload crop specialties, advertise harvest packs, and
                    coordinate freight requests with carriers.
                  </p>
                </div>
              </button>
              ...
              >
                ...
                <div>
                  <h3 className="font-bold text-slate-850 text-base">
                    Start as a Transporter
                  </h3>
                  <p className="text-xs text-slate-500 mt-1">
                    Bid on active cargo manifests, register fleet capacity,
                    upload KYC, and secure regional transit contracts.
                  </p>
                </div>
              </button>
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    Locate prime smallholder crops, publish buying targets, list
                    market stand adverts, and request direct carriers.
                  </p>
                </div>
              </button>
            </div>
            ...
            <form
              onSubmit={async (e) => {
                ...
                if (!loginPhone || loginPin.length !== 6) {
                  setLoginError(
                    "SADC Telephone and exact 6-digit PIN are required.",
                  );
                  return;
                }
                const phoneRegex = /^\+?\d{8,15}$/;
                ...
                if (foundUser) {
                  ...
                  if (decryptedPin === loginPin) {
                    ...
                    setLoginError(
                      "Access Denied: Incorrect PIN. Profile isolation secured.",
                    );
                  }
                } else {
                  setLoginError(
                    "No SADC profile found matching this telephone number.",
                  );
                }
              }}
            ...
            >
              <h3 className="text-sm font-bold text-slate-850 flex items-center gap-1.5 border-b border-slate-100 pb-2">
                <Shield size={16} className="text-emerald-600" /> Secure Device
                Access
              </h3>
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-200 focus:border-emerald-500 rounded-xl py-2.5 pl-10 pr-4 text-xs font-medium outline-none text-slate-800"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-200 focus:border-emerald-500 rounded-xl py-2.5 pl-10 pr-4 text-xs font-medium outline-none text-slate-800 animate-none"
                  />
                </div>
              </div>
              ...
              <button
                type="submit"
                className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2.5 rounded-xl text-xs shadow-md transition-colors flex items-center justify-center gap-2 mt-4 block"
              >
                <Shield size={14} />
              ...
              <p className="text-[10px] text-slate-400 leading-normal text-center mt-2">
                All profile details are stored directly on your device.
                Authenticate using your PIN to unlock securely.
              </p>
            </form>
          )}
          ...
          <button
            onClick={() => setViewState("landing")}
            className="w-full bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold py-3 rounded-xl text-xs transition-colors mt-8 flex justify-center items-center"
          >
        ...
        <div className="max-w-md mx-auto w-full px-6 py-10 flex-grow flex flex-col justify-center">
          <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-6">
            <h2 className="text-lg font-bold text-slate-850 flex items-center gap-1.5 border-b border-slate-100 pb-3">
              <Sprout size={20} className="text-emerald-600" /> Farmers
              Onboarding
            </h2>
            <p className="text-[11px] text-slate-500 mt-1 mb-5">
              Create your regional farming profile. Information remains
              encrypted locally.
            </p>
            ...
            <form onSubmit={submitFarmerOnboarding} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <input
                    required
                    type="email"
                    placeholder="e.g. tendai@farm.co"
                    value={farmerEmail}
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    value={onboardingPin}
                    onChange={(e) =>
                      setOnboardingPin(e.target.value.replace(/\D/g, ""))
                    }
                    className="w-full bg-slate-50 border border-slate-200 focus:border-emerald-500 rounded-xl p-2.5 pl-10 text-xs outline-none"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <input
                  required
                  type="text"
                  placeholder="e.g. Bindura Section B, Zimbabwe"
                  value={farmAddress}
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  value={cropSpecializations}
                  onChange={(e) => setCropSpecializations(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 focus:border-emerald-500 rounded-xl p-2.5 text-xs outline-none"
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <div className="border border-dashed border-slate-350 bg-slate-50 hover:bg-slate-100 transition-colors rounded-xl p-4 flex flex-col items-center relative overflow-hidden h-24 justify-center">
                  ...
                  <input
                    ...
                    onChange={handleProfileImageCapture}
                  />
                </div>
              </div>
              ...
              <button
                type="submit"
                className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2.5 rounded-xl text-xs shadow-md transition-colors mt-6"
              >
      ...
      {viewState === "onboard_transporter" && (
        <div className="max-w-md mx-auto w-full px-6 py-10 flex-grow flex flex-col justify-center">
          <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-6">
            <h2 className="text-lg font-bold text-slate-850 flex items-center gap-1.5 border-b border-slate-100 pb-3">
              <Truck size={20} className="text-indigo-600" /> Transporters
              Onboarding
            </h2>
            <p className="text-[11px] text-slate-500 mt-1 mb-5">
              Set up your local haulage identity. Next, submit mandatory KYC
              documents to unlock active cargos.
            </p>
            ...
            <form onSubmit={submitTransporterOnboarding} className="space-y-4">
              <div className="space-y-1">
                ...
                <input
                  required
                  type="text"
                  placeholder="e.g. Chipo Logistics Ltd"
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <input
                    required
                    type="email"
                    placeholder="e.g. chipo@cargo.sadc"
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-200 focus:border-indigo-500 rounded-xl p-2.5 pl-10 text-xs outline-none"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1">
                <label className="text-[10px] uppercase tracking-wider font-bold text-slate-500">
                  Home Fleet Hub / Depot Location
                </label>
                <input
                  required
                  type="text"
                  placeholder="e.g. Beitbridge Logistics Park, Zimbabwe"
                  value={transporterLocation}
                  onChange={(e) => setTransporterLocation(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 focus:border-indigo-500 rounded-xl p-2.5 text-xs outline-none"
                />
              </div>
              ...
              <div className="bg-amber-50 rounded-xl border border-amber-200 p-3 flex gap-2 text-[10px] text-amber-800 leading-normal">
                ...
                />
                <span>
                  You will enter driver onboarding in{" "}
                  <strong>verification pending mode</strong>. Security rules
                  require uploading fleet & license KYC to match high-value
                  Smallholder crops.
                </span>
              </div>
              ...
              <button
                type="submit"
                className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2.5 rounded-xl text-xs shadow-md transition-colors mt-4"
              >
      ...
      {viewState === "onboard_dealer" && (
        <div className="max-w-md mx-auto w-full px-6 py-10 flex-grow flex flex-col justify-center">
          <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-6">
            <h2 className="text-lg font-bold text-slate-850 flex items-center gap-1.5 border-b border-slate-100 pb-3">
              <Store size={20} className="text-amber-650" /> SADC Trade Dealer
              Onboarding
            </h2>
            <p className="text-[11px] text-slate-500 mt-1 mb-5">
              Open buying hub accounts. Match yields from smallholders directly
            ...
            <form onSubmit={submitDealerOnboarding} className="space-y-4">
              <div className="space-y-1">
                <label className="text-[10px] uppercase tracking-wider font-bold text-slate-500">
                  Hub / Outlet Register Name
                </label>
                <input
                  required
                  type="text"
                  placeholder="e.g. SADC Grain Traders"
                  value={dealerName}
                  onChange={(e) => setDealerName(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 focus:border-amber-500 rounded-xl p-2.5 text-xs outline-none"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <input
                    required
                    type="email"
                    placeholder="e.g. buy@sadctrade.co"
                    value={dealerEmail}
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-200 focus:border-amber-500 rounded-xl p-2.5 pl-10 text-xs outline-none"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1">
                <label className="text-[10px] uppercase tracking-wider font-bold text-slate-500">
                  Retail Depot / Base Location
                </label>
                <input
                  required
                  type="text"
                  placeholder="e.g. Harare Fruit & Crop Market, Zimbabwe"
              ...
              <div className="space-y-1">
                <label className="text-[10px] uppercase tracking-wider font-bold text-slate-500">
                  Primary crop varieties looking to procure
                </label>
                <input
                  required
                  type="text"
                  placeholder="e.g. Maize, Wheat, Cow Peas, Soybeans"
                  value={cropsLookingFor}
                  onChange={(e) => setCropsLookingFor(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 focus:border-amber-500 rounded-xl p-2.5 text-xs outline-none"
                />
              </div>
              ...
              <button
                type="submit"
                className="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold py-2.5 rounded-xl text-xs shadow-md transition-colors mt-6"
              >
          ...
          <div className="flex-grow w-full max-w-7xl mx-auto flex flex-col md:flex-row pb-12">
                  ...
                  <div className="flex items-center gap-2.5">
                    <div className="w-10 h-10 rounded-full border-2 border-emerald-500 overflow-hidden bg-white flex items-center justify-center text-lg flex-shrink-0">
                      {currentUser.profileImage ? (
                        ...
                        "🌾"
                      ) : currentUser.userRole === "Transporter" ? (
                        "🚚"
                      ) : (
                        "🏬"
                      )}
                    ...
                    <div className="min-w-0">
                      <h4 className="font-extrabold text-xs text-slate-800 leading-none truncate">
                        {currentUser.name}
                      </h4>
                      <div className="flex items-center gap-1.5 mt-1">
                        ...
                        <span className="text-[8px] font-mono text-emerald-600 font-bold uppercase">
                          ID: {String(currentUser.id).substring(2, 8)}
                        </span>
                      </div>
                    </div>
                  </div>
            ...
            <div className="flex-grow flex flex-col min-w-0">
              ...
              {mobileMenuOpen && (
                <div className="md:hidden bg-white border-b border-slate-205 shadow-md p-4 space-y-4 z-40 text-left">
                  ...
                  <div className="bg-slate-50 border border-slate-150 p-3 rounded-2xl flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <h4 className="font-extrabold text-xs text-slate-850 leading-none truncate">
                        {currentUser.name}
                      </h4>
                      <p className="text-[9px] text-slate-450 mt-1 font-medium">
              ...
              <div
                ...
                id="dashboard-main-render"
              >
                {activeTab === "activity" && (
                  <SADCFeedHub currentUser={currentUser} />
                )}
                {activeTab === "members" && (
                  <SADCMembers currentUser={currentUser} />
                )}

                {activeTab === "dashboard" && (
                  <>
                    {currentUser.userRole === "Farmer" && (
                      <FarmerView user={currentUser} activeTab="dashboard" />
                    )}
                    {currentUser.userRole === "Transporter" && (
                      <TransporterView
                        user={currentUser}
                    ...
                    {currentUser.userRole === "Dealer" && (
                      <DealerView user={currentUser} activeTab="dashboard" />
                    )}
                  </>
                )}
                ...
                {activeTab === "profile" && (
                  <>
                    {currentUser.userRole === "Farmer" && (
                      <FarmerView user={currentUser} activeTab="profile" />
                    )}
                    {currentUser.userRole === "Transporter" && (
                      <TransporterView user={currentUser} activeTab="profile" />
                    )}
                    {currentUser.userRole === "Dealer" && (
                      <DealerView user={currentUser} activeTab="profile" />
                    )}
                  </>
                )}
              </div>
